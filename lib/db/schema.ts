...
export const dailyAssessments = pgTable('daily_assessments', {
  ...
  imageUrl: text('image_url'), // Optional activity photo stored in object storage
  createdBy: uuid('created_by').references(() => users.id),
  createdAt: timestamp('created_at', { withTimezone: true }).defaultNow(),
});
...
export const lessonPlans = pgTable('lesson_plans', {
  ...
  classroomId: uuid('classroom_id').references(() => classrooms.id),
  date: date('date'),
  topic: text('topic'), // Main topic/theme of the lesson plan
  subtopic: text('subtopic'), // Subtopic of the lesson plan
  code: text('code'), // Optional code/identifier for the lesson
